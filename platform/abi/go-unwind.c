...
#define runtime_g runtime_getg

static void go_unwind_debug_byte(unsigned char value) {
  uint32_t eax = 63;
  uint32_t ebx = 1;
  uint32_t ecx = (uint32_t)value;
...
static void go_unwind_debug_cstring(const char* value) {
  if (value == NULL) {
    return;
  }
  while (*value != '\0') {
    go_unwind_debug_byte((unsigned char)*value++);
  }
}

static void go_unwind_debug_newline(void) {
  go_unwind_debug_byte('\r');
  go_unwind_debug_byte('\n');
}

static void go_unwind_debug_hex32(uint32_t value) {
  static const char digits[] = "0123456789ABCDEF";
  int shift;
  go_unwind_debug_cstring("0x");
  for (shift = 28; shift >= 0; shift -= 4) {
    go_unwind_debug_byte((unsigned char)digits[(value >> shift) & 0x0F]);
  }
}

static void go_unwind_debug_mark(const char* tag) {
  if (tag == NULL) {
    return;
  }
  go_unwind_debug_cstring(tag);
  go_unwind_debug_newline();
}

static void runtime_printf(const char* fmt, ...) {
  (void)fmt;
}

static void runtime_throw(const char* s) {
  (void)s;
  abort();
}

static void runtime_scanstackblockwithmap(uintptr ip, uintptr sp, uintptr size, uint8* ptrmask, void* gcw) {
  (void)ip;
  (void)sp;
  (void)size;
  (void)ptrmask;
...
static int runtime_xadd(int* addr, int delta) {
  if (addr == NULL) {
    return 0;
  }
  *addr += delta;
...
static bool runtime_usestackmaps = false;
static int __go_runtime_in_callers = 0;
...
#endif
{
  static uint32_t personality_calls = 0;
  ...
  if (personality_calls < 16) {
    debug_now = 1;
    personality_calls++;
    go_unwind_debug_cstring("P:enter a=");
    go_unwind_debug_hex32((uint32_t)actions);
    go_unwind_debug_cstring(" v=");
    go_unwind_debug_hex32((uint32_t)version);
    go_unwind_debug_newline();
  }
#endif
  ...
  if (debug_now)
    go_unwind_debug_mark("P:notable");
  CONTINUE_UNWINDING;
 ...
 found_something:
  ...
  if (actions & _UA_SEARCH_PHASE)
      ...
      if (action_record == 0)
	{
	  /* This indicates a cleanup rather than an exception
	     handler.  */
	  if (debug_now)
	    go_unwind_debug_mark("P:cleanup");
	  CONTINUE_UNWINDING;
	}

      if (debug_now)
        go_unwind_debug_mark("P:handler");
  ...
  if (debug_now)
    go_unwind_debug_mark("P:install");
