...
// The generic Null[T] helper from newer upstream Go releases is omitted here
// because the current bootstrap gccgo toolchain does not support generics yet.
type atomicError struct {
	ptr unsafe.Pointer
}

func (a *atomicError) Load() *error {
	return (*error)(atomic.LoadPointer(&a.ptr))
}

func (a *atomicError) Store(err *error) {
	atomic.StorePointer(&a.ptr, unsafe.Pointer(err))
}
...
type Rows struct {
	...
	contextDone atomicError // error that awaitDone saw; set before close attempt
