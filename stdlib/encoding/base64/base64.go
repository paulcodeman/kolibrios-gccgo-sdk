...
// AppendEncode appends the base64 encoding of src to dst and returns the updated slice.
func (enc *Encoding) AppendEncode(dst, src []byte) []byte {
	...
	origLen := len(dst)
	if cap(dst)-origLen < n {
		ndst := make([]byte, origLen, origLen+n)
		copy(ndst, dst)
		dst = ndst
	}
	dst = dst[:origLen+n]
	enc.Encode(dst[origLen:], src)
	return dst
}
...
// AppendDecode appends the base64 decoding of src to dst and returns the updated slice.
// It returns any decoding error after appending the bytes successfully decoded.
func (enc *Encoding) AppendDecode(dst, src []byte) ([]byte, error) {
	...
	dst = dst[:origLen+n]
	nw, err := enc.Decode(dst[origLen:], src)
	dst = dst[:origLen+nw]
	return dst, err
}
