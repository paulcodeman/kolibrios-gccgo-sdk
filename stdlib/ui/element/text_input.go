...
func (element *Element) Handle(event Event) bool {
	...
	if element.handleClickWithSpec(clickEvent) {
		handled = true
	}
	return handled
}
...
func (element *Element) HandleKey(key kos.KeyEvent) bool {
	...
	if element.handleKeyWithSpec(key) {
		return true
	}
	return handled
}

func (element *Element) handleTextInputKeyAction(key kos.KeyEvent) bool {
	if element == nil || !element.focused || !element.isTextInput() {
		return false
	}
...
func (element *Element) HandleScroll(deltaX int, deltaY int) bool {
	...
	if element.handleScrollWithSpec(deltaX, deltaY) {
		handled = true
	}
	return handled
}

func (element *Element) handleTextInputScrollAction(deltaX int, deltaY int) bool {
	if element == nil || !element.isTextInput() {
		return false
	}
	style := element.effectiveStyle()
