...
func colorToRGB(color kos.Color) (int, int, int) {
	value := uint32(color)
...
func (canvas *Canvas) blendPixel(dst uint32, colorValue uint32, alpha uint8) uint32 {
	if canvas == nil || !canvas.alpha {
...
func fillSlice32(slice []uint32, value uint32) {
	for i := range slice {
		slice[i] = value
	}
}
