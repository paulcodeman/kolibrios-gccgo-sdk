...
// List shows values with single or multiple selection.
type List struct {
	Values   []*ListValue
	Multiple bool
	Font     *draw.Font                              `json:"-"`
...
func (ui *List) Mouse(dui *DUI, self *Kid, m draw.Mouse, origM draw.Mouse, orig image.Point) (r Result) {
	prevM := ui.m
	ui.m = m
	if !m.In(rect(ui.size)) || len(ui.Values) == 0 {
		return
	}
	font := ui.font(dui)
	...
	if index >= len(ui.Values) {
		index = len(ui.Values) - 1
	}
