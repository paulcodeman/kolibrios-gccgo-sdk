...
import (
	...
	browserfs "github.com/psilva261/mycel/browser/fs"
...
func (el *Element) AutoMargin() (left bool, right bool) {
	if el == nil {
		return false, false
	}
	...
	return box.AutoMarginLeft, box.AutoMarginRight
}
...
func newBoxElement(n *nodes.Node, force bool, uis ...duit.UI) (box *duitx.Box, ok bool) {
	...
	var w int
	var mw int
	var autoMarginLeft bool
	var autoMarginRight bool
	...
	box = &duitx.Box{
		...
		AutoMarginRight: autoMarginRight,
		Dir:             duitFlexDir(n),
		Disp:            duitDisplay(n),
	}
...
func autoMargins(n *nodes.Node) (left bool, right bool) {
	if n == nil {
		return false, false
	}
	parseAuto := func(v string) bool {
		return strings.EqualFold(strings.TrimSpace(v), "auto")
	}
	left = parseAuto(n.Css("margin-left"))
	right = parseAuto(n.Css("margin-right"))
	if left || right {
		return left, right
	}
	parts := strings.Fields(strings.ToLower(strings.TrimSpace(n.Css("margin"))))
	switch len(parts) {
	case 1:
		left = parseAuto(parts[0])
		right = left
	case 2:
		left = parseAuto(parts[1])
		right = left
	case 3:
		left = parseAuto(parts[1])
		right = left
	default:
		if len(parts) >= 4 {
			right = parseAuto(parts[1])
			left = parseAuto(parts[3])
		}
	}
	return left, right
}
...
func (el *Element) mouseSelect(dui *duit.DUI, self *duit.Kid, m draw.Mouse, origM draw.Mouse, orig image.Point) (consumed bool) {
	leftDrag := fromLabel != nil &&
		(origM.Buttons&duit.Button1 == duit.Button1 || m.Buttons&duit.Button1 == duit.Button1) &&
		m.Point != origM.Point
	changed := false
	if leftDrag {
	...
	} else if selected > 0 && origM.Buttons&duit.Button1 == 0 && m.Buttons&duit.Button1 == duit.Button1 {
...
func NewBrowser(_dui *duit.DUI, initUrl string) (b *Browser) {
	...
	tr.Progress = func(message string) {
		if message == "" {
			return
		}
		select {
		case b.StatusCh <- message:
		default:
		}
	}
...
func (b *Browser) LinkedUrl(addr string) (a *url.URL, err error) {
	...
	if parsed, parseErr := url.Parse(addr); parseErr == nil && parsed != nil && parsed.Scheme != "" {
		return parsed, nil
	}
...
func (b *Browser) Cancel() {
	atomic.AddUint32(&b.loadSeq, 1)
	if b.cancel != nil {
		b.cancel()
	}
	b.loading = false
	select {
	case b.StatusCh <- "Canceled":
	default:
	}
}
...
func (b *Browser) currentLoadSeq() uint32 {
	return atomic.LoadUint32(&b.loadSeq)
}

func (b *Browser) showBodyMessage(msg string, loadSeq uint32) {
	if dui == nil {
		return
	}
	dui.Call <- func() {
		if loadSeq != 0 && loadSeq != b.currentLoadSeq() {
			return
		}
		b.Website.UI = &duit.Label{
			Text: msg,
...
func (b *Browser) LoadUrl(url *url.URL) (e duit.Event) {
	...
	loadSeq := atomic.AddUint32(&b.loadSeq, 1)
	b.ctx, b.cancel = context.WithCancel(context.Background())
	b.loading = true
	select {
	case b.StatusCh <- fmt.Sprintf("Load %v...", url):
	default:
	}
	go b.loadUrl(url, loadSeq)
	e.Consumed = true

	return
}

func (b *Browser) loadUrl(url *url.URL, loadSeq uint32) {
	...
	if contentType.IsHTML() || contentType.IsPlain() || contentType.IsEmpty() {
		b.render(contentType, buf, loadSeq)
	} else {
...
func (b *Browser) get(uri *url.URL, isNewOrigin bool) (buf []byte, contentType mycel.ContentType, err error) {
	if uri != nil && strings.EqualFold(uri.Scheme, "about") {
		if isNewOrigin {
			of := 0
			if scroller != nil {
				of = scroller.Offset
			}
			b.History.Push(uri, of)
			b.LocCh <- b.URL().String()
		}
		return builtinPage(uri)
	}
...
func (b *Browser) doRequest(req *http.Request) (*http.Response, error) {
	...
	go func() {
		...
		results <- requestResult{resp: resp, err: err}
	}()
...
func builtinPage(uri *url.URL) ([]byte, mycel.ContentType, error) {
	page := "blank"
	if uri != nil {
		if uri.Opaque != "" {
			page = uri.Opaque
		} else if uri.Path != "" {
			page = strings.TrimPrefix(uri.Path, "/")
		}
	}
	switch strings.ToLower(page) {
	case "", "blank":
		return []byte("<html><body></body></html>"), mycel.ContentType{MediaType: "text/html", Params: map[string]string{"charset": "utf-8"}}, nil
	case "welcome", "start", "home":
		return []byte(`<html><body><h1>Mycel</h1><p>Type a URL and press Enter.</p><p>Examples:</p><ul><li><a href="https://example.com">https://example.com</a></li><li><a href="https://example.org">https://example.org</a></li><li><a href="about:blank">about:blank</a></li></ul></body></html>`), mycel.ContentType{MediaType: "text/html", Params: map[string]string{"charset": "utf-8"}}, nil
	default:
		return []byte("<html><body><h1>about:" + page + "</h1><p>Page not found.</p></body></html>"), mycel.ContentType{MediaType: "text/html", Params: map[string]string{"charset": "utf-8"}}, nil
	}
}
